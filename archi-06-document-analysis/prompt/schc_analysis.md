...
- **Draft under study:** `{{DRAFT_TITLE}}`
- **Draft URL or text:** `{{DRAFT_URL_OR_TEXT}}`
- **Reference architecture:** `draft-ietf-schc-architecture-06`
- **Reference architecture URL or text:** `{{SCHC_ARCHITECTURE_URL_OR_TEXT}}`
...
```
# Architectural alignment review: {{DRAFT_TITLE}}
...
- **If None:** the file contains only a title and the single statement:
  `No modification to SCHC Architecture -06 is required for {{DRAFT_TITLE}}.`
...
```
# SCHC Architecture -06 edits needed for {{DRAFT_TITLE}}
